...
public class DataHelperSQLiteDAO<T> implements AJIDAO<T> {
     ...
     * @throws AJAppException: Error al asociar la clase con la tabla
     */
    public DataHelperSQLiteDAO(Class<T> dtoClass, String tableName, String tablePK) throws AJAppException {
        try {
            openConnection();
        } catch (SQLException e) {
            throw new AJAppException(null, e, getClass(), "DataHelperSQLiteDAO");
        }
    ...
    @Override
    public boolean create(T entity) throws AJAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new AJAppException(null, e, getClass(), "create");
        }
    }

    @Override
    public boolean update(T entity) throws AJAppException {
        try {
            Field[] fields = DTOClass.getDeclaredFields();
            ...
            throw new AJAppException(null, e, getClass(), "update");
        }
    }

    @Override
    public boolean delete(Integer id) throws AJAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new AJAppException(null, e, getClass(), "delete");
        }
    }

    @Override
    public T readBy(Integer id) throws AJAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new AJAppException(null, e, getClass(), "readBy");
        }
    }

    @Override
    public List<T> readAll() throws AJAppException {
    ...
    @Override
    public Integer getMaxReg(String tableCelName) throws AJAppException {
        String sql = String.format("SELECT MAX(%s) FROM %s WHERE Estado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
                ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new AJAppException(null, e, getClass(), "getMaxReg(...)");
        }
    }

    @Override
    public Integer getMinReg(String tableCelName) throws AJAppException {
        String sql = String.format("SELECT MIN(%s) FROM %s WHERE Estado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
                ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new AJAppException(null, e, getClass(), "getMinReg(...)");
        }
    }

    @Override
    public Integer getCountReg() throws AJAppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE Estado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
                ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new AJAppException(null, e, getClass(), "getCountReg(...)");
        }
    }

    protected T mapResultSetToEntity(ResultSet rs) throws AJAppException {
        try {
            ...
            throw new AJAppException(null, e, getClass(), "mapResultSetToEntity");
        }
    }
}
